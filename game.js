...
function updateDifficulty(channel) {
  ...
  // 🔥 PLAYER-BASED SCALING (MORE PLAYERS = SLOWER INCREASE)
  if (playerCount <= 4) {
    wordsPerLevel = 4;
  } 
  ...
  if (game.lastAnnouncedLength !== newLength) {
    ...
    if (newLength === 7) {
      ...
      channel.send("💀 NIGHTMARE MODE!");
    } 
    else if (newLength === 10) {
      channel.send("☠️ IMPOSSIBLE MODE!");
    } 
    else if (newLength === 12) {
      channel.send("👑 GOD MODE!");
    }
...
async function handleMessage(msg) {
  ...
  msg.channel.send(
    `✅ **${formatted}** accepted! Next: **${word.slice(-1).toUpperCase()}**`
  );
